...
show(
    beam,
    filename=output / f'{beam}gev-cumulative-bkgd-fit.pdf',
...
bkgd_yield['sim'] = integrated_bkg.value.tolist()
bkgd_yield['fit_val'] = fit_val.tolist()
bkgd_yield['up'] = up.tolist()
bkgd_yield['lo'] = lo.tolist()
# print(up, fit_val, lo)
mplhep.histplot(
    fit_val,
    bins=ana_bins,
...
# plt.ylim(ymin=1)
plt.xlim(0,cut)
show(beam, display=False, filename=output / f'{beam}gev-integrated-bkgd-fit.pdf')
